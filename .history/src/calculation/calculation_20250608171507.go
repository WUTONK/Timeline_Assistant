...
func isStartAndEndTimeHit(TimeLineStartTime string, TaskStartTime time.Time, TaskEndTime time.Time, TimeInterval int, TimeFormat string) bool {
...
// 参数 count: 计数（未使用）
// 参数 TimeFormat: 时间格式
func DisplayTaskTimeLine(t TaskList, TimeLineStartTime string, TimeInterval int, count int, TimeFormat string) {

	Tasks := make([]*Task, 0, 10)          // 任务列表
	InfoRightList := make([]string, 0, 10) // 右侧信息列表
	InfoLeftList := make([]string, 0, 10)  // 左侧信息列表（时间轴）

	// 将任务存储到切片中
	for CurrentTask := t.TaskHead; CurrentTask != nil; {
		Hit := isStartAndEndTimeHit(TimeLineStartTime, CurrentTask.Task_Start_Time, CurrentTask.Task_End_Time, TimeInterval, TimeFormat)
		if Hit {
	...
	CurrencyTime, _ := time.Parse(TimeFormat, TimeLineStartTime)
	...
	for i := 0; ; {
		...
		for TaskIndex, Task := range Tasks {
			...
			if CurrentInfoRight != "nil" {
				InfoRightList = append([i]InfoRightList, CurrentInfoRight)
			}
		}
...
// 显示右侧信息
func DisplayTaskTimeLine_Right(Index int, Task *Task, CurrencyTime time.Time) string {
	...
	if TaskStatus != "DontNeedDisplay" {
		InfoRight := fmt.Sprintf("<--%s %s", Task.Task_Name, TaskStatus)
		return InfoRight
	}
	return "nil"
}
