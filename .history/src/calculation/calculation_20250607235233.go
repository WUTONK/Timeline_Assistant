...
func AddTask(t TaskList, TaskName string, TaskStartTime time.Time, TaskEndTime time.Time) {
	NewTask := &Task{Task_Name: TaskName, Task_Start_Time: TaskStartTime, Task_End_Time: TaskEndTime}
...
func DisplayTaskTimeLine(t TaskList, StartTime string, TimeInterval int, count int, TimeFormat string) {
	// 创建一个切片来存储任务
	tasks := make([]Task, 0, 10) // 初始容量为10，但长度为0的切片

	// 遍历链表，将任务添加到切片中
	currentTask := t.TaskHead
	for currentTask != nil {
		tasks = append(tasks, *currentTask) // 将当前任务添加到切片中
		currentTask = currentTask.Next_Task
	}

	// 现在可以通过索引访问任务，例如：tasks[0], tasks[1] 等
	// 示例：打印第一个任务
	if len(tasks) > 0 {
		fmt.Printf("第一个任务: %s\n", tasks[0].Task_Name)
	}
