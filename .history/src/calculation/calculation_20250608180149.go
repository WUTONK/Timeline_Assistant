...
func DisplayTaskTimeLine_isStartAndEndTimeHit(TimeLineStartTime string, TaskStartTime time.Time, TaskEndTime time.Time, TimeInterval int, TimeFormat string) bool {
	...
	// 计算任务开始和结束时间的小时和分钟
	startHour := TaskStartTime.Hour()
	startMin := TaskStartTime.Minute()
	endHour := TaskEndTime.Hour()
	endMin := TaskEndTime.Minute()

	// 遍历24小时
	for i := 0; i < 1440/TimeInterval; i++ {
		checkTime := CurrentTime.Add(time.Duration(i*TimeInterval) * time.Minute)
		checkHour := checkTime.Hour()
		checkMin := checkTime.Minute()

		// 检查是否命中开始时间
		if checkHour == startHour && checkMin == startMin {
			isStartHit = true
		}
		// 检查是否命中结束时间
		if checkHour == endHour && checkMin == endMin {
			isEndHit = true
		}

		// 如果两个时间都命中了，就可以返回true
		if isStartHit && isEndHit {
			return true
		}
	}

	fmt.Printf("任务时间：%02d:%02d - %02d:%02d 未命中时间线\n",
