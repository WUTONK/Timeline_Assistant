...
func DisplayTaskTimeLine(t TaskList, StartTime string, TimeInterval int, count int, TimeFormat string) {
	...
	newFunction(tasks, TimeFormat)
}

func newFunction(tasks []*Task, TimeFormat string) {
	for i, task := range tasks {
		fmt.Printf("任务 %d: %s 开始时间%v 结束时间%v\n", i, task.Task_Name, task.Task_Start_Time.Format(TimeFormat), task.Task_End_Time.Format(TimeFormat))
	}
}
