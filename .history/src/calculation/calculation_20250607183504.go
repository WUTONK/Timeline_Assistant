...
func PrintTimeList_new(StartTime string, TimeInterval int, count int, TimeFormat string) {
	...
	isTimeLessthenSource := false
	isTimeGreaterthanSource := false

	for i := 0; !TimeHasCycle; {

		if CurrentTimeCopyOne.Add(time.Duration(i*TimeInterval)*time.Minute) == CurrentTime {
			fmt.Println("一次跳点刚好24小时,该情况下无法判断是在时钟是否经过一周")
			return
		}
		...
		if !isTimeLessthenSource {
			if TimeOneSumMinute < TimeTwoSumMinute {
				isTimeLessthenSource = true
			}
		} else {
			if TimeOneSumMinute >= TimeTwoSumMinute {
				TimeHasCycle = true
